    ...
    while True:
        ...
        if(self_robot_id and nowtime > last_move_time + 0.01):
            ...
            if(key_press[K_LEFT]):
                ...
                client.send(msg.encode("utf-8"))
            elif (key_press[K_q] and nowtime > last_missile_time + 5):
                last_missile_time = nowtime
                msg = str(self_robot_id) + " 7"
                client.send(msg.encode("utf-8"))
            elif (key_press[K_r]):
                last_shoot_time = nowtime
                msg = str(self_robot_id) + " 90"
                client.send(msg.encode("utf-8"))
        ...
        for mis in missile_list:
            if(mis.z == self_level):
                if(self_robot_id == mis.fa):
                    pygame.draw.rect(screen,colorper,Rect(mis.x,mis.y,10,10))
                else:
                    pygame.draw.rect(screen,colorother,Rect(mis.x,mis.y,10,10))
            else:
                pygame.draw.rect(screen,colorlow,Rect(mis.x,mis.y,10,10))
